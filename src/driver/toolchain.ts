...
/** Locate a clang-compatible C compiler, honouring `XTSC_CLANG`. */
export function findClang(runner: Runner = realRunner): string {
  const candidates = [
    process.env.XTSC_CLANG,
    "clang",
    "clang-18",
    "clang-17",
    "clang-16",
  ...
  throw new ToolchainError("clang --version", 1, "No C compiler found. Set XTSC_CLANG to a clang binary.");
}
